...
def main(output_path=None) -> None:
    ...
    phi0 = sdf_circle(grid, (0.0, 0.0), 0.55)
    result = evolve_level_set(
        phi0,
        grid.spacing,
        velocity_fn=lambda phi, _t: isotropic_etch_rate(phi, 0.1),
        t_final=0.4,
    )
    out = output_dir(output_path)
    save_npz(out / "isotropic_etch_2d.npz", phi0=phi0, phi=result.phi)
    save_optional_contour(out / "isotropic_etch_2d.png", result.phi, "Isotropic etch 2D")
    print(f"devices: {describe_devices()}")
    print(f"steps: {result.num_steps}, dt: {result.dt:.6g}")


if __name__ == "__main__":
    args = example_parser("run isotropic 2D etch example").parse_args()
